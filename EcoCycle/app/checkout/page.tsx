...
interface OrderItem {
  ...
  productId: number;    // Product ID ← สำคัญ
  name: string;
  quantity: number;
  price: number;
  imageUrl: string;
...
const CheckoutPage = () => {
      ...
      .then(data => {
        const items = data.map((item: any) => {
          ...
          return {
            ...
            productId: item.productId, // ✅ Important: used in checkout payload
            name: item.name,
  ...
  const handleCheckout = async () => {
    ...
    const payload = {
      ...
      items: orderItems.map(({ productId, quantity, price }) => ({
        id: productId,  // ✅ ส่ง key ชื่อ id (ตรงกับ backend)
        quantity,
        price,
      })),
      subtotal,
  ...
  // สไตล์ที่ใช้ร่วมกันสำหรับช่อง input
